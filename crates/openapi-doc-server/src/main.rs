...
async fn handle_index(State(state): State<AppState>) -> Result<Html<String>, StatusCode> {
    ...
    let apis: Vec<ServerApiDocEntry> = cache.values()
...
async fn handle_api_request(
    ...
    if let Some(api_json) = cache.get(decoded_name_str) {
        ...
        if let Ok(api_entry) = serde_json::from_str::<ServerApiDocEntry>(api_json) {
            // Parse the spec using shared utility
            let spec = spec_utils::parse_spec_to_json(&api_entry.spec).unwrap_or_else(|_| serde_json::Value::Null);
            Ok(Json(spec))
        } else {
...
fn generate_index_html(apis: &[ServerApiDocEntry]) -> String {
    ...
    for (i, api) in apis.iter().enumerate() {
        ...
        let api_spec: serde_json::Value = spec_utils::parse_spec_to_json(&api.spec)
            .unwrap_or_else(|_| serde_json::from_str(&spec_utils::create_default_spec(&api.name, "API documentation not available")).unwrap_or(serde_json::Value::Null));
