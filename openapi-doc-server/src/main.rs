...
struct ApiDocEntry {
    ...
    service_name: String,
    url: String,
    description: Option<String>, // Changed from String to Option<String> to match operator
    last_updated: String,
    available: bool,
    spec: String, // The actual OpenAPI spec content
}

#[derive(Debug, Clone, serde::Deserialize, serde::Serialize)]
struct DiscoveryConfig {
    apis: Vec<ApiDocEntry>,
    last_updated: String, // This will be deserialized as string from the operator's DateTime<Utc>
}
...
async fn handle_index(State(state): State<AppState>) -> Result<Html<String>, StatusCode> {
    ...
    let apis: Vec<ApiDocEntry> = cache.values()
        .filter_map(|json| serde_json::from_str::<ApiDocEntry>(json).ok())
        .collect();
...
async fn handle_api_request(
    ...
    tracing::info!("Looking for API: '{}' (decoded: '{}')", api_name, decoded_name_str);
    tracing::info!("Available APIs in cache: {:?}", cache.keys().collect::<Vec<_>>());
    ...
    if let Some(api_json) = cache.get(decoded_name_str) {
        ...
        // Parse the API entry to get the spec
        if let Ok(api_entry) = serde_json::from_str::<ApiDocEntry>(api_json) {
            // Parse the spec as YAML and convert to JSON
            let spec: serde_json::Value = if api_entry.spec.trim().starts_with('{') {
                // Already JSON
                serde_json::from_str(&api_entry.spec).unwrap_or_else(|_| serde_json::Value::Null)
            } else {
                // YAML content, parse it
                serde_yaml::from_str(&api_entry.spec).unwrap_or_else(|_| serde_json::Value::Null)
            };
            Ok(Json(spec))
        } else {
...
fn generate_index_html(apis: &[ApiDocEntry]) -> String {
    ...
    for (i, api) in apis.iter().enumerate() {
        // Parse the API spec
        let api_spec: serde_json::Value = if let Ok(spec) = serde_json::from_str(&api.spec) {
            spec
        } else if let Ok(spec) = serde_yaml::from_str(&api.spec) {
            spec
        } else {
            serde_json::json!({
                "openapi": "3.0.0",
...
async fn refresh_api_cache(state: &AppState) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match fs::read_to_string(discovery_path) {
        Ok(discovery_json) => {
            let discovery_config: DiscoveryConfig = serde_json::from_str(&discovery_json)?;
